...
public class WFX_Demo : MonoBehaviour
{
	...
	private List<string> groundTextures = new List<string>(new[]{"Concrete","Wood","Metal","Checker"});
	...
	void OnGUI()
	{
		...
		*/
		...
		if(GUILayout.Button(GetComponent<Renderer>().enabled ? "Hide Ground" : "Show Ground", GUILayout.Width(90)))
		{
	...
	LOOP:
		...
		if(orderedSpawns)
		{
			particles.transform.position = transform.position + new Vector3(order,particles.transform.position.y,0);
			order -= step;
			if(order < -range) order = range;
		}
		else 				particles.transform.position = transform.position + new Vector3(Random.Range(-range,range),0,Random.Range(-range,range)) + new Vector3(0,particles.transform.position.y,0);
